...
def get_lane_metrics(lane_id):
    """Metrics without detectors: Use lane and vehicle APIs only"""
    try:
        ...
        F = vehicle_count * 360
        # Downstream congestion detection (if any links are >80% occupied)
        ...
        print(f"⚠️  Không thể lấy metrics từ {lane_id}: {e}")
...
def plot_congestion_graph(tracking_data, tl_id):
    try:
        ...
        ax.set_title(f'Trạng Thái Giao Thông: Chỉ Số Tắc Nghẽn Theo Thời Gian (Đèn {tl_id})', fontsize=15)
